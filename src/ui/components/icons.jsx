import {
  Target,
  Sword,
  ArrowLeftRight,
  Rocket,
  Eye,
  Scan,
  Radio,
  ZoomIn,
  HelpCircle,
  Users,
  Globe,
  Database,
  Map,
  Package,
  Circle,
  PenLine,
  Settings,
  Layers,
  RotateCw,
  BadgePlus,
} from 'lucide-preact';

const Surplus = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="currentColor" {...props}>
    {/* Face background */}
    <circle cx="50" cy="50" r="45" fill="#FFD700" stroke="#FFA500" strokeWidth="2"/>
    
    {/* Left eye */}
    <circle cx="35" cy="38" r="6" fill="#333333"/>
    <circle cx="35" cy="38" r="2.5" fill="#FFFFFF" cx="36" cy="37"/>
    
    {/* Right eye */}
    <circle cx="65" cy="38" r="6" fill="#333333"/>
    <circle cx="65" cy="38" r="2.5" fill="#FFFFFF" cx="66" cy="37"/>
    
    {/* Mouth - curved smile */}
    <path d="M 35 62 Q 50 75 65 62" stroke="#333333" strokeWidth="2.5" fill="none" strokeLinecap="round"/>
    
    {/* Chin/Hand - finger on chin */}
    <g transform="translate(70, 65)">
      {/* Arm */}
      <path d="M 0 0 Q 15 -5 20 5" stroke="#FFD700" strokeWidth="6" fill="none" strokeLinecap="round"/>
      {/* Hand/Finger */}
      <circle cx="22" cy="8" r="5" fill="#FFAA44"/>
      <circle cx="22" cy="8" r="3" fill="#FFD700"/>
    </g>
  </svg>
);

const Discord = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}>
    <path d="M20.317 4.4917C18.7873 3.8008 17.147 3.2918 15.4319 3C15.4007 2.9952 15.3695 3.0096 15.3534 3.0384C15.1424 3.4077 14.9087 3.8948 14.7451 4.2875C12.9004 4.0141 11.0652 4.0141 9.25832 4.2875C9.09465 3.8862 8.85248 3.4077 8.64057 3.0384C8.62449 3.0105 8.59328 2.9961 8.56205 3C6.84791 3.2909 5.20756 3.7999 3.67693 4.4917C3.66368 4.4973 3.65233 4.5065 3.64479 4.5185C0.533392 9.2227 -0.31895 13.8151 0.0991801 18.3525C0.101072 18.3736 0.11337 18.3938 0.130398 18.4075C2.18321 19.9061 4.17171 20.8159 6.12328 21.4179C6.15451 21.4275 6.18761 21.4161 6.20748 21.3899C6.66913 20.7693 7.08064 20.1152 7.43348 19.4271C7.4543 19.3873 7.43442 19.3402 7.39186 19.3245C6.73913 19.0763 6.1176 18.7745 5.51973 18.4321C5.47244 18.4046 5.46865 18.3366 5.51216 18.3043C5.63797 18.2104 5.76382 18.1128 5.88396 18.0142C5.90569 17.9961 5.93598 17.9923 5.96153 18.0038C9.88928 19.789 14.1415 19.789 18.023 18.0038C18.0485 17.9914 18.0788 17.9952 18.1015 18.0133C18.2216 18.1118 18.3475 18.2104 18.4742 18.3043C18.5177 18.3366 18.5149 18.4046 18.4676 18.4321C17.8697 18.7812 17.2482 19.0763 16.5945 19.3236C16.552 19.3393 16.533 19.3873 16.5538 19.4271C16.9143 20.1143 17.3258 20.7684 17.7789 21.3889C17.7978 21.4161 17.8319 21.4275 17.8631 21.4179C19.8241 20.8159 21.8126 19.9061 23.8654 18.4075C23.8834 18.3938 23.8948 18.3745 23.8967 18.3534C24.3971 13.1418 23.0585 8.5868 20.3482 4.5194C20.3416 4.5065 20.3303 4.4973 20.317 4.4917ZM8.02002 15.5869C6.8375 15.5869 5.86313 14.515 5.86313 13.1932C5.86313 11.8714 6.8186 10.7995 8.02002 10.7995C9.23087 10.7995 10.1958 11.8809 10.1769 13.1932C10.1769 14.515 9.22141 15.5869 8.02002 15.5869ZM15.9947 15.5869C14.8123 15.5869 13.8379 14.515 13.8379 13.1932C13.8379 11.8714 14.7933 10.7995 15.9947 10.7995C17.2056 10.7995 18.1705 11.8809 18.1516 13.1932C18.1516 14.515 17.2056 15.5869 15.9947 15.5869Z" />
  </svg>
);

const PenLineFilled = (props) => <PenLine {...props} strokeWidth="2.5" />;

export const Icons = {
  Aimbot_: Target,
  MeleeLock_: Sword,
  AutoSwitch_: ArrowLeftRight,
  SemiAuto_: Rocket,
  XRay_: Eye,
  ESP_: Radio,
  Help_: HelpCircle,
  Community_: Users,
  Website_: Globe,
  Credits_: Database,
  Map_: Map,
  AutoLoot_: Package,
  Surplus_: BadgePlus,
  Discord_: Discord,
  Pen_: PenLineFilled,
  Main_: Settings,
  Visuals_: Eye,
  Misc_: Layers,
  Spinbot_: RotateCw,
};
